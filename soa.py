...
def index():
    ...
    return render_template('index.html', user = user, name = None)
    ...
    if form.validate_on_submit():
        ...
        if user is not None:
            if user.password == form.password.data:
                ...
                login_user(user)
                return render_template('index.html', name = session.get('name'), user = user)
        else:
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        if user is None and email is None:
            ...
            login_user(user)
            return render_template('index.html', form=form, user = user, name = session.get('name'))
        else:
            if user is not None:
...
def upload(): 
    ...
    if form.validate_on_submit():
        ...
        if flag == 1: 
            ...
            resource = LResource(name = form.name.data, types = int(types), format = int(format), course_id = int(course_id), url = form.url.data, author = form.author.data, uploader_id = int(uploader_id), desc = form.desc.data, addr = address)
...
def updateResource():
    ...
    for resource in resources:
        ...
        #up_id.append(User.query.filter_by(id = resource.id).first().username)
    ...
    return render_template('viewResource.html', length = length, up_time = up_time, rids = rid, resources = name, res_desc = desc, res_type = res_type, res_format = res_format, name = session.get('name'))
...
def deleteStd():
    std = StandardAndSpe.query.filter_by(id = request.form['stdandard']).first()
    if std != None:
